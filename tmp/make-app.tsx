...
function GlowCursor() {
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      if (starRef.current) { starRef.current.style.left = `${x}px`; starRef.current.style.top = `${y}px`; }
      if (glowRef.current) { glowRef.current.style.left = `${x}px`; glowRef.current.style.top = `${y}px`; }
    };
    const onOver = (e: MouseEvent) => {
      const hov = !!(e.target as HTMLElement).closest("a,button,[role=button],label");
      if (starRef.current) {
        starRef.current.style.width  = hov ? "36px" : "24px";
        starRef.current.style.height = hov ? "36px" : "24px";
        starRef.current.style.filter = hov
          ? "drop-shadow(0 0 8px rgba(255,160,32,1)) drop-shadow(0 0 16px rgba(255,100,0,.8))"
          : "drop-shadow(0 0 5px rgba(255,160,32,.8)) drop-shadow(0 0 10px rgba(255,100,0,.5))";
      }
    };
    const spin = () => {
      rotRef.current += 0.6;
      if (starRef.current) {
        const cx = parseFloat(starRef.current.style.left) || 0;
        const cy = parseFloat(starRef.current.style.top)  || 0;
        starRef.current.style.transform = `translate(-50%,-50%) rotate(${rotRef.current}deg)`;
      }
  ...
  const base: React.CSSProperties = { position: "fixed", pointerEvents: "none", zIndex: 99999, top: 0, left: 0 };
  // 六芒星 = 两个等边三角形叠加，用 polygon path
  const hex = "M12,0 L14.5,8.5 L23,8.5 L16.5,13.5 L19,22 L12,17 L5,22 L7.5,13.5 L1,8.5 L9.5,8.5 Z";

  return (
    <>
      <div ref={glowRef} style={{ ...base, width: 480, height: 480, borderRadius: "50%", transform: "translate(-50%,-50%)",
        background: "radial-gradient(circle,rgba(255,138,31,.07) 0%,transparent 65%)", transition: "left .18s ease,top .18s ease" }} />
      <svg ref={starRef} viewBox="0 0 24 24" style={{
        ...base, width: 24, height: 24,
        transform: "translate(-50%,-50%)",
        filter: "drop-shadow(0 0 5px rgba(255,160,32,.8)) drop-shadow(0 0 10px rgba(255,100,0,.5))",
        transition: "width .2s, height .2s, filter .2s",
        overflow: "visible",
      }}>
        {/* 六芒星：△ 朝上 */}
        <polygon points="12,2 22,19 2,19"
          fill="rgba(255,150,40,.12)" stroke="rgba(255,180,60,.9)" strokeWidth="1.1" strokeLinejoin="round" />
        {/* 六芒星：▽ 朝下 */}
        <polygon points="12,22 2,5 22,5"
          fill="rgba(255,150,40,.12)" stroke="rgba(255,180,60,.9)" strokeWidth="1.1" strokeLinejoin="round" />
        {/* 中心亮点 */}
        <circle cx="12" cy="12" r="1.6" fill="#ffdd80" />
      </svg>
    </>
  );
}
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: "#090A0E" }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden" style={{ minWidth: 0 }}>
        {/* ── Header ── */}
        <header style={{
          height: (currentPage === "plot-analysis" || currentPage === "plot-analysis-detail") ? 0 : 60,
          display: (currentPage === "plot-analysis" || currentPage === "plot-analysis-detail") ? "none" : "flex",
          alignItems: "center",
        ...
        <main className={`flex-1 ${(currentPage === "plot-analysis" || currentPage === "plot-analysis-detail") ? "overflow-hidden flex flex-col" : "overflow-y-auto"}`}>{children}</main>
      </div>
    </div>
  );
}
...
// ─── AssetPage ────────────────────────────────────────────────────────────────
function AssetPage({ navigate }: Nav) {
  type Comp  = "pass"|"review"|"generating"|"failed"|"empty";
  type RW    = "idle"|"loading"|"comparing"|"applied";
  type DView = "front"|"side"|"back"|"outfit"|"detail";
  type AD = { id:string; name:string; desc:string; files:number; compliance:Comp; refs:number; expiry:string; image?:string; imageStyle?:string; historyImages:string[]; };

  const [tab,        setTab]        = useState<"chars"|"scenes"|"props">("chars");
  const [moreId,     setMoreId]     = useState<string|null>(null);
  const [sel,        setSel]        = useState<AD|null>(null);
  const [dView,      setDView]      = useState<DView>("front");
  const [histIdx,    setHistIdx]    = useState(0);
  const [editName,   setEditName]   = useState(false);
  const [aName,      setAName]      = useState("");
  const [prompt,     setPrompt]     = useState("");
  const [rw,         setRw]         = useState<RW>("idle");
  const [ratio,      setRatio]      = useState("3:4");
  const [sty,        setSty]        = useState("写实");
  const [expandOpen, setExpandOpen] = useState(false);
  const [expandText, setExpandText] = useState("");
  const [expandDirty,setExpandDirty]= useState(false);
  const [showDiscard,setShowDiscard]= useState(false);
  const [rwDrawer,   setRwDrawer]   = useState(false);
  const [rwInput,    setRwInput]    = useState("");
  const [genModel,   setGenModel]   = useState("Flux 1.1 Pro");
  const [genQuality, setGenQuality] = useState("标准");
  const MODELS    = ["Flux 1.1 Pro","Flux Dev","SDXL Turbo","Stable Cascade"];
  const QUALITIES = ["草稿","标准","高清","超清"];

  type VoiceTab    = "system"|"custom";
  type VoiceAge    = "儿童"|"少年"|"青年"|"中年"|"老年";
  type VoiceGender = "男"|"女";
  type VoiceLang   = "中文"|"英文";
  const [voiceOpen,  setVoiceOpen]  = useState(false);
  const [voiceAsset, setVoiceAsset] = useState<AD|null>(null);
  const [voiceTab,   setVoiceTab]   = useState<VoiceTab>("system");
  const [selVoice,   setSelVoice]   = useState<string|null>(null);
  const [creating,   setCreating]   = useState(false);
  const [cvName,     setCvName]     = useState("");
  const [cvGender,   setCvGender]   = useState<VoiceGender>("女");
  const [cvAge,      setCvAge]      = useState<VoiceAge>("青年");
  const [cvLang,     setCvLang]     = useState<VoiceLang>("中文");
  const [cvTraining, setCvTraining] = useState(false);
  const [cvFile,     setCvFile]     = useState<string|null>(null);
  const AGE_OPTS: VoiceAge[]   = ["儿童","少年","青年","中年","老年"];
  const LANG_OPTS: VoiceLang[] = ["中文","英文"];
  const SYS_VOICES = [
    { id:"sv1", name:"晓辰",  gender:"女" as VoiceGender, age:"青年" as VoiceAge, lang:"中文" as VoiceLang, tag:"温柔知性" },
    { id:"sv2", name:"云逸",  gender:"男" as VoiceGender, age:"青年" as VoiceAge, lang:"中文" as VoiceLang, tag:"沉稳大气" },
    { id:"sv3", name:"小艾",  gender:"女" as VoiceGender, age:"少年" as VoiceAge, lang:"中文" as VoiceLang, tag:"活泼清亮" },
    { id:"sv4", name:"Marcus",gender:"男" as VoiceGender, age:"青年" as VoiceAge, lang:"英文" as VoiceLang, tag:"专业播报" },
    { id:"sv5", name:"Echo",  gender:"女" as VoiceGender, age:"青年" as VoiceAge, lang:"英文" as VoiceLang, tag:"自然流畅" },
    { id:"sv6", name:"墨渊",  gender:"男" as VoiceGender, age:"中年" as VoiceAge, lang:"中文" as VoiceLang, tag:"磁性低沉" },
  ];
  const CUSTOM_VOICES = [
    { id:"uv1", name:"林凯专属", gender:"男" as VoiceGender, age:"青年" as VoiceAge, lang:"中文" as VoiceLang },
  ];
  const openVoice = (e: React.MouseEvent, a: AD) => {
    e.stopPropagation();
    setVoiceAsset(a); setVoiceTab("system"); setCreating(false);
    setCvName(""); setCvFile(null); setCvTraining(false); setVoiceOpen(true);
  };
  const closeVoice = () => { setVoiceOpen(false); setCreating(false); };

  const ORIG_PROMPT = `角色设定：都市知性白领女性，28岁左右，气质优雅中带着干练。\n\n外貌特征：鹅蛋脸型，柳叶眉，杏仁眼，高鼻梁，嘴唇饱满自然。黑色直发，长至肩胛骨，发梢有自然弧度。肤色白皙，无明显妆容，仅有淡淡口红。\n\n服装造型：深藏青色西装外套，内搭白色丝质衬衫，细腰带系腰，整体利落而不失女性柔美。搭配中跟黑色尖头皮鞋。\n\n姿态要求：站姿端正，双手自然垂放，目光直视镜头，表情沉静而有力量感。`;
  const REWR_PROMPT = `【主体】都市精英女性，28岁，气质内敛锐利\n【外貌】鹅蛋脸，柳叶眉，双眼皮杏眼，高鼻梁；黑色直发及肩胛，发梢内卷，肤色白净\n【服装】深藏青西装（细密纹理）+ 白丝衬衫 + 细腰带，配中跟黑色尖头皮鞋\n【姿态】正面站立，双手自然垂放，目视镜头，表情沉静有力\n【风格】写实主义，细节精准，无过度美化\n【背景】纯色深灰/浅灰渐变，便于抠图合成`;

  type AssetPrompt = { id: string; label: string; text: string };
  const CHAR_PORTRAIT_PROMPT = `纯白背景，不出现文字。\n【左半部分】：2x2网格排列的同一女性角色的四张头部肖像图：\n图1（左上）：正面头部特写，展示面部特征和发型。\n图2（右上）：顺时针45度侧脸头部特写。\n图3（左下）：逆时针45度侧脸头部特写。\n图4（右下）：正面胸像，展示角色穿着该服装领口效果。\n【右半部分】：展示该服装的去头三视图（正面、侧面、背面）：必须完全移除头部，颈部领口直接空出。必须保留完整身体、双臂和双腿。\nSoft diffused studio lighting with zero oily reflections.`;
  const CHAR_OUTFIT_PROMPT = `纯白背景，服装平铺展示，正面·侧面·背面三视图并排，完整展示服装结构与面料细节，无人物，仅服装本体，标注材质，高精度棚拍质感，电商产品级渲染。`;

  const [multiPrompts, setMultiPrompts] = useState<AssetPrompt[]>([]);
  const [activePromptIdx, setActivePromptIdx] = useState(0);

  const f1    = "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=480&h=640&fit=crop&auto=format";
  const f2    = "https://images.unsplash.com/photo-1675726205553-4e348f24da2c?w=480&h=640&fit=crop&auto=format";
  const m1    = "https://images.unsplash.com/photo-1570216601541-fa11cfaf03e5?w=480&h=640&fit=crop&auto=format";
  const f1lg  = "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=900&h=1100&fit=crop&auto=format";
  const f2lg  = "https://images.unsplash.com/photo-1675726205553-4e348f24da2c?w=900&h=1100&fit=crop&auto=format";
  const m1lg  = "https://images.unsplash.com/photo-1570216601541-fa11cfaf03e5?w=900&h=1100&fit=crop&auto=format";
  const cityS = "https://images.unsplash.com/photo-1679212839469-fb16a48919ce?w=480&h=640&fit=crop&auto=format";
  const cit2S = "https://images.unsplash.com/photo-1535391879778-3bae11d29a24?w=480&h=640&fit=crop&auto=format";
  const cityL = "https://images.unsplash.com/photo-1679212839469-fb16a48919ce?w=1200&h=800&fit=crop&auto=format";
  const cit2L = "https://images.unsplash.com/photo-1535391879778-3bae11d29a24?w=1200&h=800&fit=crop&auto=format";

  const assets: Record<string, AD[]> = {
    chars: [
      { id:"c1", name:"陈默",    desc:"女主角 · 内敛冷静",  files:8, compliance:"pass",       refs:23, expiry:"15天", image:f1,  historyImages:[f1lg,f2lg,f1lg,f2lg] },
      { id:"c2", name:"林凯",    desc:"男主角 · 温柔执着",  files:5, compliance:"review",     refs:18, expiry:"8天",  image:m1,  historyImages:[m1lg,m1lg] },
      { id:"c3", name:"方远",    desc:"反派 · 城府极深",    files:0, compliance:"generating", refs:0,  expiry:"–",    historyImages:[] },
      { id:"c4", name:"警卫甲",  desc:"配角 · 出现第3集",   files:2, compliance:"failed",     refs:0,  expiry:"3天",  historyImages:[] },
      { id:"c5", name:"神秘来客",desc:"待定 · 尚未定义",    files:0, compliance:"empty",      refs:0,  expiry:"–",    historyImages:[] },
    ],
    scenes: [
      { id:"s1", name:"公司走廊", desc:"主要场景 · 冷色调大理石", files:12, compliance:"pass",       refs:34, expiry:"22天", image:cityS, historyImages:[cityL,cit2L,cityL] },
      { id:"s2", name:"天台",     desc:"关键场景 · 傍晚霞光",     files:9,  compliance:"pass",       refs:15, expiry:"10天", image:cit2S, historyImages:[cit2L,cityL] },
      { id:"s3", name:"审讯室",   desc:"常用场景 · 室内灯光",     files:0,  compliance:"generating", refs:0,  expiry:"–",    historyImages:[] },
    ],
    props: [
      { id:"p1", name:"手机",    desc:"道具 · 未接来电截图", files:4, compliance:"pass",   refs:31, expiry:"20天", imageStyle:"linear-gradient(145deg,#1a1a1a 0%,#2c2c2c 35%,#4a4a4a 60%,#333 100%)", historyImages:[] },
      { id:"p2", name:"文件夹",  desc:"道具 · 机密工作文件", files:3, compliance:"review", refs:7,  expiry:"5天",  imageStyle:"linear-gradient(145deg,#1a0800 0%,#3d1500 40%,#6b2500 65%,#3d1500 100%)", historyImages:[] },
      { id:"p3", name:"保险箱",  desc:"道具 · 关键线索",     files:1, compliance:"failed", refs:0,  expiry:"2天",  historyImages:[] },
    ],
  };

  const tabs     = [{ k:"chars" as const, l:"人物", icon:User }, { k:"scenes" as const, l:"场景", icon:MapPin }, { k:"props" as const, l:"道具", icon:Package }];
  const addLabel = { chars:"新增人物", scenes:"新增场景", props:"新增道具" };
  ...
  const styles   = ["写实","水墨","3D渲染","漫画"];
  const MAX_P    = 2000;

  const compCfg: Record<Comp,{text:string;c:string;bg:string;border:string}> = {
    pass:       { text:"合规",     c:"#34d399", bg:"rgba(52,211,153,.1)",  border:"rgba(52,211,153,.25)"  },
    ...
    failed:     { text:"生成失败", c:"#f87171", bg:"rgba(248,113,113,.1)", border:"rgba(248,113,113,.25)" },
    empty:      { text:"未生成",   c:"#71717a", bg:"rgba(113,113,122,.1)", border:"rgba(113,113,122,.25)" },
  };

  function openAsset(a: AD) {
    setSel(a); setDView("front"); setHistIdx(0);
    const baseText = a.id==="c1" ? ORIG_PROMPT : `${a.name}：${a.desc}`;
    setPrompt(baseText);
    if (a.type === "character") {
      setMultiPrompts([
        { id: "mp1", label: "4格肖像参考图", text: CHAR_PORTRAIT_PROMPT },
        { id: "mp2", label: "三视图服装展示", text: CHAR_OUTFIT_PROMPT },
        { id: "mp3", label: "完整角色提示词", text: baseText },
      ]);
    } else if (a.type === "scene") {
      setMultiPrompts([
        { id: "mp1", label: "场景全景", text: baseText },
        { id: "mp2", label: "场景细节", text: `${a.name}局部细节，高清质感，电影感构图，写实风格` },
      ]);
    } else {
      setMultiPrompts([
        { id: "mp1", label: "道具主视图", text: baseText },
        { id: "mp2", label: "道具细节", text: `${a.name}材质细节特写，产品级渲染，纯白背景` },
      ]);
    }
    setActivePromptIdx(0);
    setRw("idle"); setAName(a.name); setEditName(false); setMoreId(null);
    setRwDrawer(false); setRwInput("");
  }
  function closeModal() { setSel(null); setExpandOpen(false); setRw("idle"); setRwDrawer(false); setRwInput(""); }
  function triggerRewrite() {
    const cur = multiPrompts[activePromptIdx];
    if (!cur?.text.trim() && !prompt.trim()) return;
    setRw("loading");
    setTimeout(() => setRw("comparing"), 2200);
  }
  function applyRewrite() {
    setMultiPrompts(prev => prev.map((mp,i)=>i===activePromptIdx?{...mp,text:REWR_PROMPT}:mp));
    setPrompt(REWR_PROMPT); setRw("applied");
  }
  function openExpand()   { setExpandText(prompt); setExpandDirty(false); setShowDiscard(false); setExpandOpen(true); }
  function saveExpand()   {
    setMultiPrompts(prev => prev.map((mp,i)=>i===activePromptIdx?{...mp,text:expandText}:mp));
    setPrompt(expandText); setExpandDirty(false); setExpandOpen(false);
  }
  function tryCloseExpand() { if (expandDirty) setShowDiscard(true); else setExpandOpen(false); }

  const mainImg     = sel ? (sel.historyImages[histIdx] ?? null) : null;
  const hasVis      = (a: AD) => !!(a.image || a.imageStyle);
  const promptLen   = prompt.length;
  const promptOver  = promptLen > MAX_P;
  const expandOver  = expandText.length > MAX_P;
  const imgBg       = "#09090f";

  return (
    <div style={{ padding:"20px 28px", width:"100%", boxSizing:"border-box" }}>

      {/* Breadcrumb */}
      <div style={{ display:"flex", alignItems:"center", gap:8, fontSize:13, color:textMuted, marginBottom:14 }}>
        <button onClick={()=>navigate("projects")} style={{ background:"none",border:"none",color:textMuted,cursor:"pointer" }}>剧目创作</button>
        <ChevronRight style={{ width:13,height:13 }} />
        <button onClick={()=>navigate("projects")} style={{ background:"none",border:"none",color:textMuted,cursor:"pointer" }}>{PROJECT.name}</button>
        <ChevronRight style={{ width:13,height:13 }} />
        <span style={{ color:"rgba(255,255,255,.7)",fontWeight:500 }}>资产管理</span>
      </div>

      {/* Header */}
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14 }}>
        <div>
          <h1 style={{ fontSize:20,fontWeight:700,color:"white" }}>资产管理</h1>
          <p style={{ fontSize:13,color:textMuted,marginTop:3 }}>{PROJECT.name}</p>
        </div>
        <div style={{ display:"flex",gap:10 }}>
          <button className="glass-btn" style={{ display:"flex",alignItems:"center",gap:7,padding:"9px 14px",borderRadius:11,fontSize:13,color:textDim,border:"none" }}>
            <Upload style={{ width:14,height:14 }} />批量上传
          </button>
          <button onClick={()=>navigate("storyboard")} className="orange-btn" style={{ display:"flex",alignItems:"center",gap:7,padding:"9px 16px",borderRadius:11,fontSize:13,fontWeight:600,color:"black",border:"none" }}>
            进入分镜管理<ChevronRight style={{ width:14,height:14 }} />
          </button>
        </div>
      </div>

      {/* Warning */}
      <div style={{ background:"rgba(251,191,36,.06)",border:"1px solid rgba(251,191,36,.15)",borderRadius:12,padding:"12px 16px",display:"flex",alignItems:"flex-start",gap:8,marginBottom:16 }}>
        <AlertTriangle style={{ width:15,height:15,color:"#fbbf24",flexShrink:0,marginTop:1 }} />
        <p style={{ fontSize:12,color:"rgba(255,255,255,.45)",lineHeight:1.6 }}>素材文件为短期缓存，到期须重新上传。平台不提供永久存储，请在到期前完成生成任务。</p>
      </div>

      {/* Main panel */}
      <div style={{ background:surface,border:`1px solid ${bdr}`,borderRadius:18,overflow:"hidden",boxShadow:cardShadow }}>
          ...
          {tabs.map(t=>(
            <button key={t.k} onClick={()=>setTab(t.k)}
              style={{ flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:8,padding:"13px 0",
                ...
                color:tab===t.k?gold:textMuted,
                borderBottom:tab===t.k?`2px solid ${gold}`:"2px solid transparent" }}>
              <t.icon style={{ width:14,height:14 }} />{t.l}
              <span style={{ fontSize:11,padding:"1px 7px",borderRadius:20,
                background:tab===t.k?"rgba(255,138,31,.15)":"rgba(255,255,255,.06)",
                color:tab===t.k?gold:textMuted }}>
                {assets[tab].length}
              </span>
            </button>
        ...
        <div style={{ padding:20,display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(200px, 1fr))",gap:14 }}>
          {assets[tab].map(a=>{
            const c = compCfg[a.compliance];
            const hv = hasVis(a);
            return (
              <div key={a.id} onClick={()=>openAsset(a)}
                style={{ border:`1px solid ${bdr}`,borderRadius:14,overflow:"hidden",cursor:"pointer",
                  transition:"border-color .2s,box-shadow .2s",position:"relative",background:surface }}
                onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.borderColor="rgba(255,138,31,.35)";(e.currentTarget as HTMLElement).style.boxShadow="0 4px 24px rgba(0,0,0,.5)";}}
                onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.borderColor=bdr;(e.currentTarget as HTMLElement).style.boxShadow="none";setMoreId(null);}}>

                {/* Image area */}
                <div style={{ aspectRatio:"3/4",position:"relative",overflow:"hidden",background:imgBg }}>
                  {a.image && <img src={a.image} alt={a.name} style={{ width:"100%",height:"100%",objectFit:"cover",objectPosition:"top",display:"block" }} />}
                  {a.imageStyle && !a.image && <div style={{ width:"100%",height:"100%",background:a.imageStyle }} />}

                  {/* Generating */}
                  {!hv && a.compliance==="generating" && (
                    <div style={{ position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8 }}>
                      <div style={{ width:34,height:34,borderRadius:"50%",border:"2px solid rgba(96,165,250,.2)",borderTopColor:"#60a5fa",animation:"spin 1s linear infinite" }} />
                      <span style={{ fontSize:11,color:"#60a5fa",fontWeight:500 }}>AI 生成中</span>
                    </div>
                  )}
                  {/* Failed */}
                  {!hv && a.compliance==="failed" && (
                    <div style={{ position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8 }}>
                      <div style={{ width:34,height:34,borderRadius:"50%",background:"rgba(248,113,113,.1)",border:"1px solid rgba(248,113,113,.2)",display:"flex",alignItems:"center",justifyContent:"center" }}>
                        <AlertTriangle style={{ width:15,height:15,color:"#f87171" }} />
                      </div>
                      <span style={{ fontSize:11,color:"#f87171",fontWeight:500 }}>生成失败</span>
                      <button onClick={e=>e.stopPropagation()}
                        style={{ display:"flex",alignItems:"center",gap:4,padding:"4px 10px",fontSize:10,color:"#f87171",background:"rgba(248,113,113,.08)",border:"1px solid rgba(248,113,113,.2)",borderRadius:7,cursor:"pointer" }}>
                        <RotateCcw style={{ width:9,height:9 }} />重新生成
                      </button>
                    </div>
                  )}
                  {/* Empty */}
                  {!hv && a.compliance==="empty" && (
                    <div style={{ position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,padding:12 }}>
                      <ImageIcon style={{ width:20,height:20,color:"rgba(255,255,255,.18)" }} />
                      <span style={{ fontSize:10,color:textMuted }}>暂无图像</span>
                      <div style={{ display:"flex",flexDirection:"column",gap:5,width:"80%" }}>
                        <button onClick={e=>e.stopPropagation()} style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:4,padding:"5px 0",fontSize:10,color:textMuted,background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.09)",borderRadius:7,cursor:"pointer" }}>
                          <Upload style={{ width:9,height:9 }} />上传素材
                        </button>
                        <button onClick={e=>e.stopPropagation()} style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:4,padding:"5px 0",fontSize:10,color:gold,background:"rgba(255,138,31,.07)",border:"1px solid rgba(255,138,31,.18)",borderRadius:7,cursor:"pointer" }}>
                          <Sparkles style={{ width:9,height:9 }} />AI 生成
                        </button>
                      </div>
                    </div>
                  )}

                  {hv && <div style={{ position:"absolute",inset:0,background:"linear-gradient(to top,rgba(14,11,22,.85) 0%,transparent 55%)",pointerEvents:"none" }} />}

                  {/* Status badge */}
                  <div style={{ position:"absolute",top:8,left:8,zIndex:2 }}>
                    <span style={{ fontSize:10,fontWeight:600,padding:"2px 7px",borderRadius:20,background:c.bg,color:c.c,border:`1px solid ${c.border}`,display:"inline-flex",alignItems:"center",gap:4 }}>
                      {a.compliance==="generating" && <Loader2 style={{ width:8,height:8 }} />}
                      {c.text}
                    </span>
                  </div>
                  {/* More */}
                  <button onClick={e=>{e.stopPropagation();setMoreId(moreId===a.id?null:a.id);}}
                    style={{ position:"absolute",top:8,right:8,zIndex:2,width:26,height:26,borderRadius:7,
                    ...
                    <MoreHorizontal style={{ width:13,height:13,color:"white" }} />
                  </button>
                  {moreId===a.id && (
                    <div onClick={e=>e.stopPropagation()}
                      style={{ position:"absolute",top:36,right:8,zIndex:10,background:"#181B24",border:"1px solid rgba(255,255,255,.08)",borderRadius:12,boxShadow:"0 8px 32px rgba(0,0,0,.6)",padding:"6px 0",minWidth:116 }}>
                      {["编辑资产","重新生成","下载素材"].map(l=>(
                        <button key={l} style={{ width:"100%",display:"flex",alignItems:"center",gap:7,padding:"7px 12px",fontSize:12,color:textDim,background:"none",border:"none",cursor:"pointer" }}
                          onMouseEnter={e=>(e.currentTarget as HTMLElement).style.background="rgba(255,255,255,.05)"}
                          onMouseLeave={e=>(e.currentTarget as HTMLElement).style.background="none"}>{l}</button>
                      ))}
                      <div style={{ height:1,background:"rgba(255,255,255,.06)",margin:"4px 8px" }} />
                      <button style={{ width:"100%",display:"flex",alignItems:"center",gap:7,padding:"7px 12px",fontSize:12,color:"#f87171",background:"none",border:"none",cursor:"pointer" }}
                        onMouseEnter={e=>(e.currentTarget as HTMLElement).style.background="rgba(248,113,113,.06)"}
                        onMouseLeave={e=>(e.currentTarget as HTMLElement).style.background="none"}>删除</button>
                    </div>
                  )}
                </div>
                ...
                <div style={{ padding:"12px 14px 14px" }}>
                  <div style={{ fontSize:13.5,fontWeight:700,color:"white",marginBottom:3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{a.name}</div>
                  <div style={{ fontSize:11.5,color:textMuted,marginBottom:10,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{a.desc}</div>
                  <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:11,color:textMuted,marginBottom:10 }}>
                    <span>{a.files} 个素材</span>
                    <span>引用 {a.refs} 次</span>
                    {a.expiry!=="–" && <span style={{ color:"rgba(251,191,36,.7)" }}>剩余 {a.expiry}</span>}
                  </div>
                  <button onClick={e=>openVoice(e,a)}
                    style={{ width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"7px 0",
                      ...
                      color:textDim,transition:"all .15s" }}
                    onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background="rgba(255,138,31,.08)";(e.currentTarget as HTMLElement).style.borderColor="rgba(255,138,31,.25)";(e.currentTarget as HTMLElement).style.color=gold;}}
                    onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background="rgba(255,255,255,.05)";(e.currentTarget as HTMLElement).style.borderColor="rgba(255,255,255,.09)";(e.currentTarget as HTMLElement).style.color=textDim;}}>
                    <Mic2 style={{ width:12,height:12 }} />配置音色
                  </button>
                </div>
              </div>
            );
          ...
          {/* Add button */}
          <button style={{ border:`2px dashed rgba(255,255,255,.1)`,borderRadius:14,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:10,cursor:"pointer",minHeight:280,background:"transparent",transition:"all .15s",color:textMuted }}
            ...
            onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.borderColor="rgba(255,255,255,.1)";(e.currentTarget as HTMLElement).style.color=textMuted;}}>
            <Plus style={{ width:22,height:22 }} />
            <span style={{ fontSize:13,fontWeight:500 }}>{addLabel[tab]}</span>
          </button>
        </div>
      </div>
      ...
      {/* ── Detail modal ── */}
      {sel && (
        <div style={{ position:"fixed",inset:0,zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,.72)",backdropFilter:"blur(12px)" }}
          onClick={e=>{if(e.target===e.currentTarget)closeModal();}}>
          <div style={{ width:"min(960px,92vw)",height:"min(680px,88vh)",background:"#0d0b18",border:"1px solid rgba(255,255,255,.1)",borderRadius:24,display:"flex",overflow:"hidden",boxShadow:"0 32px 80px rgba(0,0,0,.8)",position:"relative" }}>
            <button onClick={closeModal}
              style={{ position:"absolute",top:16,right:16,zIndex:10,width:30,height:30,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(255,255,255,.08)",border:"1px solid rgba(255,255,255,.12)",color:"rgba(255,255,255,.6)",cursor:"pointer" }}>
              <X style={{ width:14,height:14 }} />
            </button>

            {/* Left: image */}
            <div style={{ width:340,flexShrink:0,background:imgBg,display:"flex",flexDirection:"column",overflow:"hidden" }}>
              {/* View tabs */}
              <div style={{ display:"flex",gap:4,padding:"12px 14px 8px",flexShrink:0 }}>
                {viewTabs.map(vt=>(
                  <button key={vt.k} onClick={()=>setDView(vt.k)}
                    style={{ flex:1,padding:"5px 0",fontSize:10.5,fontWeight:500,borderRadius:7,background:dView===vt.k?"rgba(255,138,31,.14)":"rgba(255,255,255,.05)",border:`1px solid ${dView===vt.k?"rgba(255,138,31,.35)":"rgba(255,255,255,.09)"}`,color:dView===vt.k?gold:"rgba(255,255,255,.45)",cursor:"pointer" }}>
                    {vt.l}
                  </button>
                ))}
              </div>
              ...
              <div style={{ flex:1,position:"relative",overflow:"hidden",margin:"0 14px" }}>
                {mainImg ? (
                  <img src={mainImg} alt={sel.name} style={{ width:"100%",height:"100%",objectFit:"cover",objectPosition:"top",borderRadius:12 }} />
                ) : sel.imageStyle ? (
                  <div style={{ width:"100%",height:"100%",background:sel.imageStyle,borderRadius:12 }} />
                ) : (
                  <div style={{ width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",color:"rgba(255,255,255,.2)",flexDirection:"column",gap:8,borderRadius:12,border:"1px dashed rgba(255,255,255,.1)" }}>
                    <ImageIcon style={{ width:32,height:32 }} />
                    <span style={{ fontSize:12 }}>暂无图像</span>
                  </div>
                )}
              </div>
              {/* History strip */}
              {sel.historyImages.length>0 && (
                <div style={{ display:"flex",gap:6,padding:"10px 14px",overflowX:"auto",flexShrink:0 }}>
                  {sel.historyImages.map((img,i)=>(
                    <div key={i} onClick={()=>setHistIdx(i)}
                      style={{ width:44,height:58,borderRadius:7,overflow:"hidden",flexShrink:0,cursor:"pointer",border:`2px solid ${histIdx===i?gold:"transparent"}`,transition:"border-color .15s" }}>
            ...
            <div style={{ flex:1,display:"flex",flexDirection:"column",overflow:"hidden" }}>
              {/* Name row */}
              <div style={{ padding:"16px 20px 12px",borderBottom:"1px solid rgba(255,255,255,.07)",display:"flex",alignItems:"center",gap:10,flexShrink:0 }}>
                {editName ? (
                  <input autoFocus value={aName} onChange={e=>setAName(e.target.value)}
                    onBlur={()=>setEditName(false)} onKeyDown={e=>{if(e.key==="Enter"||e.key==="Escape")setEditName(false);}}
                    style={{ fontSize:16,fontWeight:700,color:"white",background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,138,31,.4)",borderRadius:8,padding:"4px 10px",outline:"none",flex:1 }} />
                ) : (
                  <h2 onClick={()=>setEditName(true)} style={{ fontSize:16,fontWeight:700,color:"white",cursor:"text",flex:1,margin:0 }}>{aName}</h2>
                )}
                <span style={{ fontSize:11,color:textMuted,background:"rgba(255,255,255,.06)",padding:"2px 10px",borderRadius:20,flexShrink:0 }}>{sel.desc}</span>
              </div>

              {/* Prompt area */}
              <div style={{ flex:1,overflowY:"auto",padding:"14px 20px",display:"flex",flexDirection:"column",gap:12 }}>
                {/* Multi-prompt tabs */}
                <div style={{ display:"flex",alignItems:"center",gap:0,borderBottom:"1px solid rgba(255,255,255,.07)",marginBottom:2 }}>
                  {multiPrompts.map((mp, idx) => (
                    <button key={mp.id} onClick={()=>setActivePromptIdx(idx)}
                      style={{ padding:"6px 12px",fontSize:11.5,fontWeight:activePromptIdx===idx?700:400,color:activePromptIdx===idx?gold:"rgba(255,255,255,.45)",background:"none",border:"none",borderBottom:`2px solid ${activePromptIdx===idx?gold:"transparent"}`,cursor:"pointer",marginBottom:-1,whiteSpace:"nowrap",transition:"color .15s" }}>
                      {mp.label}
                    </button>
                  ))}
                  <button onClick={()=>{
                    const newId = `mp${Date.now()}`;
                    setMultiPrompts(prev => [...prev, { id:newId, label:`自定义提示词${prev.length}`, text:"" }]);
                    setActivePromptIdx(multiPrompts.length);
                  }} style={{ padding:"6px 10px",fontSize:12,color:"rgba(255,255,255,.3)",background:"none",border:"none",cursor:"pointer",marginLeft:"auto",display:"flex",alignItems:"center",gap:3 }}>
                    <Plus style={{ width:12,height:12 }} />新增
                  </button>
                </div>
                {multiPrompts[activePromptIdx] && (
                  <>
                    <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                      <span style={{ fontSize:12,fontWeight:600,color:"rgba(255,255,255,.5)" }}>{multiPrompts[activePromptIdx].label}</span>
                      <div style={{ display:"flex",gap:6 }}>
                        <span style={{ fontSize:11,color:multiPrompts[activePromptIdx].text.length>MAX_P?"#f87171":textMuted }}>{multiPrompts[activePromptIdx].text.length}/{MAX_P}</span>
                        <button onClick={()=>{ setExpandText(multiPrompts[activePromptIdx].text); setExpandDirty(false); setShowDiscard(false); setExpandOpen(true); }} style={{ fontSize:11,color:textMuted,background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",borderRadius:6,padding:"2px 9px",cursor:"pointer" }}>展开编辑</button>
                        {multiPrompts.length > 1 && (
                          <button onClick={()=>{
                            setMultiPrompts(prev => prev.filter((_,i)=>i!==activePromptIdx));
                            setActivePromptIdx(Math.max(0,activePromptIdx-1));
                          }} style={{ fontSize:11,color:"#f87171",background:"rgba(248,113,113,.08)",border:"1px solid rgba(248,113,113,.2)",borderRadius:6,padding:"2px 9px",cursor:"pointer" }}>删除</button>
                        )}
                      </div>
                    </div>
                    <textarea value={multiPrompts[activePromptIdx].text}
                      onChange={e=>{
                        const val = e.target.value;
                        setMultiPrompts(prev => prev.map((mp,i)=>i===activePromptIdx?{...mp,text:val}:mp));
                        if (activePromptIdx === multiPrompts.findIndex(mp=>mp.id==="mp3"||mp.label==="完整角色提示词")) setPrompt(val);
                      }} rows={8}
                      style={{ background:"rgba(255,255,255,.04)",border:`1px solid ${multiPrompts[activePromptIdx].text.length>MAX_P?"rgba(248,113,113,.4)":"rgba(255,255,255,.08)"}`,borderRadius:10,padding:"10px 12px",fontSize:12.5,color:"rgba(255,255,255,.8)",outline:"none",resize:"none",lineHeight:1.7,fontFamily:"inherit" }} />
                  </>
                )}

                {/* Rewrite controls */}
                {rw==="idle" && (
                  <button onClick={triggerRewrite} style={{ display:"flex",alignItems:"center",gap:7,padding:"9px 16px",borderRadius:10,fontSize:13,fontWeight:600,background:"rgba(255,138,31,.1)",border:"1px solid rgba(255,138,31,.3)",color:gold,cursor:"pointer",alignSelf:"flex-start" }}>
                    <Wand2 style={{ width:14,height:14 }} />AI 优化提示词
                  </button>
                )}
                {rw==="loading" && (
                  <div style={{ display:"flex",alignItems:"center",gap:8,color:"rgba(255,255,255,.5)",fontSize:13 }}>
                    <Loader2 style={{ width:14,height:14,animation:"spin 1s linear infinite" }} />正在分析并重写…
                  </div>
                )}
                {rw==="comparing" && (
                  <div style={{ background:"rgba(255,138,31,.06)",border:"1px solid rgba(255,138,31,.2)",borderRadius:12,padding:"12px 14px" }}>
                    <div style={{ fontSize:11.5,color:gold,marginBottom:8,fontWeight:600 }}>✨ AI 已生成优化版本</div>
                    <div style={{ fontSize:11.5,color:"rgba(255,255,255,.6)",lineHeight:1.7,marginBottom:10 }}>
                      {REWR_PROMPT.slice(0,120)}…
                    </div>
                    <div style={{ display:"flex",gap:8 }}>
                      <button onClick={applyRewrite} className="orange-btn" style={{ padding:"6px 16px",borderRadius:8,fontSize:12,fontWeight:600,color:"black",border:"none",cursor:"pointer" }}>应用此版本</button>
                      <button onClick={()=>setRw("idle")} style={{ padding:"6px 14px",borderRadius:8,fontSize:12,color:textMuted,background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.1)",cursor:"pointer" }}>放弃</button>
                    </div>
                  </div>
                )}
                {rw==="applied" && (
                  <div style={{ display:"flex",alignItems:"center",gap:6,fontSize:12,color:"#34d399" }}>
                    <Check style={{ width:13,height:13 }} />已应用 AI 优化版本
                  </div>
                )}

                {/* Gen options */}
                <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
                  <select value={ratio} onChange={e=>setRatio(e.target.value)} style={{ background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",borderRadius:8,padding:"6px 10px",fontSize:12,color:"rgba(255,255,255,.7)",outline:"none",cursor:"pointer" }}>
                    {ratios.map(r=><option key={r} style={{ background:"#0f0d1a" }}>{r}</option>)}
                  </select>
                  <select value={sty} onChange={e=>setSty(e.target.value)} style={{ background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",borderRadius:8,padding:"6px 10px",fontSize:12,color:"rgba(255,255,255,.7)",outline:"none",cursor:"pointer" }}>
                    {styles.map(s=><option key={s} style={{ background:"#0f0d1a" }}>{s}</option>)}
                  </select>
                  <select value={genModel} onChange={e=>setGenModel(e.target.value)} style={{ background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",borderRadius:8,padding:"6px 10px",fontSize:12,color:"rgba(255,255,255,.7)",outline:"none",cursor:"pointer",flex:1 }}>
                    {MODELS.map(m=><option key={m} style={{ background:"#0f0d1a" }}>{m}</option>)}
                  </select>
                  <select value={genQuality} onChange={e=>setGenQuality(e.target.value)} style={{ background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",borderRadius:8,padding:"6px 10px",fontSize:12,color:"rgba(255,255,255,.7)",outline:"none",cursor:"pointer" }}>
                    {QUALITIES.map(q=><option key={q} style={{ background:"#0f0d1a" }}>{q}</option>)}
                  </select>
                </div>
              </div>

              {/* Generate button */}
              <div style={{ padding:"12px 20px",borderTop:"1px solid rgba(255,255,255,.07)",display:"flex",alignItems:"center",gap:10,flexShrink:0 }}>
                <button className="orange-btn" style={{ flex:1,padding:"10px 0",borderRadius:12,fontSize:14,fontWeight:700,color:"black",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8 }}>
                  <Sparkles style={{ width:15,height:15 }} />生成图像
                </button>
                <button onClick={e=>openVoice(e,sel)} style={{ padding:"10px 16px",borderRadius:12,fontSize:13,fontWeight:500,background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",color:textDim,cursor:"pointer",display:"flex",alignItems:"center",gap:6 }}>
                  <Mic2 style={{ width:13,height:13 }} />音色
                </button>
              </div>
            </div>
          </div>
      ...
      {/* ── Expand prompt modal ── */}
      {expandOpen && (
        <div style={{ position:"fixed",inset:0,zIndex:600,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,.8)",backdropFilter:"blur(8px)" }}>
          <div style={{ width:"min(680px,90vw)",background:"#0d0b18",border:"1px solid rgba(255,255,255,.1)",borderRadius:20,padding:"24px",display:"flex",flexDirection:"column",gap:16,maxHeight:"80vh" }}>
            <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
              <h3 style={{ fontSize:15,fontWeight:700,color:"white",margin:0 }}>编辑提示词</h3>
              <button onClick={tryCloseExpand} style={{ background:"none",border:"none",color:textMuted,cursor:"pointer",display:"flex" }}>
                <X style={{ width:16,height:16 }} />
              </button>
            </div>
            <textarea value={expandText} onChange={e=>{setExpandText(e.target.value);setExpandDirty(true);}}
              style={{ flex:1,minHeight:320,background:"rgba(255,255,255,.04)",border:`1px solid ${expandOver?"rgba(248,113,113,.4)":"rgba(255,255,255,.08)"}`,borderRadius:12,padding:"12px 14px",fontSize:13,color:"rgba(255,255,255,.8)",outline:"none",resize:"vertical",lineHeight:1.8,fontFamily:"inherit" }} />
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
              <span style={{ fontSize:11,color:expandOver?"#f87171":textMuted }}>{expandText.length}/{MAX_P}</span>
              <div style={{ display:"flex",gap:10 }}>
                <button onClick={()=>setExpandOpen(false)} style={{ padding:"8px 18px",borderRadius:10,fontSize:13,color:textMuted,background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.1)",cursor:"pointer" }}>取消</button>
                <button onClick={saveExpand} className="orange-btn" style={{ padding:"8px 20px",borderRadius:10,fontSize:13,fontWeight:600,color:"black",border:"none",cursor:"pointer" }}>保存</button>
              </div>
            </div>
            {showDiscard && (
              <div style={{ background:"rgba(248,113,113,.08)",border:"1px solid rgba(248,113,113,.2)",borderRadius:10,padding:"12px 14px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                <span style={{ fontSize:13,color:"rgba(255,255,255,.7)" }}>有未保存的修改，确定放弃？</span>
                <div style={{ display:"flex",gap:8 }}>
                  <button onClick={()=>setShowDiscard(false)} style={{ padding:"5px 14px",borderRadius:8,fontSize:12,color:textMuted,background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.1)",cursor:"pointer" }}>继续编辑</button>
                  <button onClick={()=>{setShowDiscard(false);setExpandOpen(false);}} style={{ padding:"5px 14px",borderRadius:8,fontSize:12,color:"#f87171",background:"rgba(248,113,113,.08)",border:"1px solid rgba(248,113,113,.2)",cursor:"pointer" }}>放弃修改</button>
                </div>
              </div>
            )}
          </div>
      ...
      {voiceOpen && voiceAsset && (
        <div style={{ position:"fixed",inset:0,zIndex:700,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,.75)",backdropFilter:"blur(10px)" }}
          onClick={e=>{if(e.target===e.currentTarget)closeVoice();}}>
          <div style={{ width:"min(560px,92vw)",background:"#0e0c1a",border:"1px solid rgba(255,255,255,.1)",borderRadius:22,padding:"24px",display:"flex",flexDirection:"column",gap:16,maxHeight:"80vh",overflowY:"auto" }}>
            <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
              <div>
                ...
                <div style={{ fontSize:12,color:textMuted,marginTop:2 }}>{voiceAsset.name} · {voiceAsset.desc}</div>
              </div>
              <button onClick={closeVoice} style={{ background:"none",border:"none",color:textMuted,cursor:"pointer",display:"flex" }}>
                <X style={{ width:16,height:16 }} />
              </button>
            </div>
            ...
            {/* Tabs */}
            <div style={{ display:"flex",gap:6 }}>
              {(["system","custom"] as VoiceTab[]).map(vt=>(
                <button key={vt} onClick={()=>{setVoiceTab(vt);setCreating(false);setSelVoice(null);}}
                  ...
                  <button key={v.id} onClick={()=>setSelVoice(v.id)}
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ display:"flex",gap:8,alignItems:"center",marginBottom:3 }}>
                        ...
                        <span style={{ fontSize:10.5,color:textMuted,background:"rgba(255,255,255,.06)",padding:"1px 7px",borderRadius:20 }}>{v.tag}</span>
                      </div>
                      <div style={{ fontSize:11,color:textMuted }}>{v.gender} · {v.age} · {v.lang}</div>
                    </div>
            ...
            {voiceTab==="custom" && !creating && (
              <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
                ...
                <button onClick={()=>setCreating(true)}
                  style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:7,padding:"10px",borderRadius:12,background:"rgba(255,255,255,.03)",border:"2px dashed rgba(255,255,255,.1)",color:textMuted,fontSize:13,cursor:"pointer" }}>
            ...
            {voiceTab==="custom" && creating && (
              <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
                  ...
                  {(["男","女"] as VoiceGender[]).map(g=>(
                    <button key={g} onClick={()=>setCvGender(g)}
                      style={{ flex:1,padding:"7px",borderRadius:9,fontSize:13,fontWeight:500,cursor:"pointer",
                        ...
                        color:cvGender===g?gold:"rgba(255,255,255,.5)" }}>{g}</button>
                  ))}
                  {AGE_OPTS.map(a=>(
                    <button key={a} onClick={()=>setCvAge(a)}
                      style={{ flex:1,padding:"7px",borderRadius:9,fontSize:11,fontWeight:500,cursor:"pointer",
                        ...
                        color:cvAge===a?gold:"rgba(255,255,255,.5)" }}>{a}</button>
                  ))}
                </div>
                <label style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:8,padding:"24px",borderRadius:12,cursor:"pointer",
                  background:"rgba(255,255,255,.03)",border:`2px dashed ${cvFile?"rgba(255,138,31,.4)":"rgba(255,255,255,.1)"}`,color:cvFile?gold:textMuted,fontSize:13 }}>
                ...
                <div style={{ display:"flex",gap:8 }}>
                  <button onClick={()=>setCreating(false)} style={{ flex:1,padding:"9px",borderRadius:10,fontSize:13,color:textMuted,background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.1)",cursor:"pointer" }}>取消</button>
                  <button onClick={()=>{if(cvFile&&cvName)setCvTraining(true);}} disabled={!cvFile||!cvName}
                    className="orange-btn" style={{ flex:2,padding:"9px",borderRadius:10,fontSize:13,fontWeight:600,color:"black",border:"none",cursor:"pointer",opacity:cvFile&&cvName?1:.4 }}>
                    开始克隆
                  </button>
                </div>
              </div>
            ...
            {/* Apply button */}
            {!creating && (
              <button onClick={closeVoice} disabled={!selVoice} className="orange-btn"
...
const APP_PAGES: PageId[] = ["workspace","projects","new-project","ai-chapter","assets","storyboard","batch","result","delivery","billing","canvas","user-center","plot-analysis","plot-analysis-detail","team","team-assets"];
...
// ─── TeamTab ──────────────────────────────────────────────────────────────────
const TEAM_ROLES = ["管理员", "导演", "编剧", "制作人"] as const;
...
function TeamTab() {
  ...
  const MAX = 5;
  const isOwner = true; /* 当前登录用户是工作室管理员 */
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {/* Header card */}
      <div style={{ background: surface, border: `1px solid ${bdr}`, borderRadius: 12, padding: "20px 24px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: "white", marginBottom: 3 }}>工作室成员</h3>
            <p style={{ fontSize: 12, color: textMuted }}>
              {members.length}/{MAX} 人 · 仅管理员可邀请或移除成员
            </p>
          </div>
          {isOwner && members.length < MAX && (
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 16px", borderRadius: 10,
                ...
                color: gold, fontSize: 13, fontWeight: 600, cursor: "none" }}>
              <Plus style={{ width: 14, height: 14 }} />
              邀请成员
            </button>
          )}
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          {members.map((m, i) => {
            ...
            const isCreator = m.id === TEAM_OWNER_ID;
            return (
              <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "13px 0",
                borderBottom: i < members.length - 1 ? "1px solid rgba(255,255,255,.05)" : "none" }}>
                {/* Avatar */}
                <div style={{ width: 38, height: 38, borderRadius: "50%", flexShrink: 0,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  background: rs.bg, color: rs.color, fontSize: 14, fontWeight: 700 }}>
                  {m.avatar}
                </div>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                    <span style={{ fontSize: 13.5, fontWeight: 600, color: "white" }}>{m.name}</span>
                    <span style={{ fontSize: 10.5, fontWeight: 600, padding: "2px 8px", borderRadius: 100,
                      background: rs.bg, color: rs.color }}>{m.role}</span>
                    {m.id === members[0].id && (
                      <span style={{ fontSize: 10, color: "rgba(255,255,255,.3)" }}>（我）</span>
                    )}
                  </div>
                  <div style={{ display: "flex", gap: 12 }}>
                    <span style={{ fontSize: 11, color: "rgba(255,255,255,.28)" }}>{m.account}</span>
                    <span style={{ fontSize: 11, color: "rgba(255,255,255,.2)" }}>加入 {m.joined}</span>
                  </div>
                </div>
                ...
                {isOwner && !isCreator && (
                  removeId === m.id ? (
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <span style={{ fontSize: 11.5, color: "rgba(255,255,255,.5)" }}>确认移除？</span>
                      <button onClick={() => handleRemove(m.id)}
                        style={{ fontSize: 11.5, color: "#f87171", background: "rgba(248,113,113,.1)",
                          border: "1px solid rgba(248,113,113,.25)", padding: "4px 10px",
                          borderRadius: 8, fontWeight: 600, cursor: "none" }}>移除</button>
                      <button onClick={() => setRemoveId(null)}
                        style={{ fontSize: 11.5, color: textMuted, background: "none",
                          border: "none", padding: "4px 6px", cursor: "none" }}>取消</button>
                    </div>
                  ) : (
                    <button onClick={() => setRemoveId(m.id)}
                      style={{ fontSize: 11.5, color: "rgba(255,255,255,.3)", background: "none",
                        border: "1px solid rgba(255,255,255,.08)", padding: "5px 12px",
      ...
      <div style={{ background: surface, border: `1px solid ${bdr}`, borderRadius: 16, padding: "16px 20px" }}>
        <h4 style={{ fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,.4)", marginBottom: 14,
          letterSpacing: "0.05em", textTransform: "uppercase" as const }}>角色权限说明</h4>
        {/* Permission table */}
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr>
                <th style={{ textAlign: "left", padding: "6px 10px", color: "rgba(255,255,255,.3)", fontWeight: 600, fontSize: 11, borderBottom: "1px solid rgba(255,255,255,.06)", width: "44%" }}>权限</th>
                {(["所有者","管理员","制作人"] as const).map(r => {
                  const rs = r === "所有者" ? { bg: "rgba(255,138,31,.18)", color: "#FF8A1F" } : ROLE_STYLE[r] ?? ROLE_STYLE["制作人"];
                  return (
                    <th key={r} style={{ textAlign: "center", padding: "6px 8px", borderBottom: "1px solid rgba(255,255,255,.06)" }}>
                      <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 100, background: rs.bg, color: rs.color }}>{r}</span>
                    </th>
                  );
                })}
              </tr>
            ...
            <tbody>
              {[
                { perm: "使用团队资产（角色/场景/道具）",  owner: true,  admin: true,  maker: true  },
                { perm: "进入和共创团队项目",              owner: true,  admin: true,  maker: true  },
                { perm: "取得生成权后发起生成",            owner: true,  admin: true,  maker: true  },
                ...
                { perm: "充值、升级团队套餐",              owner: true,  admin: true,  maker: false },
                { perm: "强制释放项目生成锁",              owner: true,  admin: true,  maker: false },
                { perm: "解散团队、转移所有权",            owner: true,  admin: false, maker: false },
              ].map((row, i) => (
                <tr key={row.perm} style={{ borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                  <td style={{ padding: "8px 10px", color: "rgba(255,255,255,.5)", fontSize: 11.5 }}>{row.perm}</td>
                  {[row.owner, row.admin, row.maker].map((has, j) => (
                    <td key={j} style={{ textAlign: "center", padding: "8px 8px" }}>
                      {has
                        ? <span style={{ fontSize: 13, color: "#34d399" }}>✓</span>
                        : <span style={{ fontSize: 13, color: "rgba(255,255,255,.12)" }}>—</span>}
                    </td>
                  ))}
                </tr>
              ))}
        ...
        <p style={{ fontSize: 11, color: "rgba(255,255,255,.2)", marginTop: 12, lineHeight: 1.6 }}>
          * 导演/编剧具有制作人相同权限 · 团队余额不足时任务提交失败，不自动扣个人余额
        </p>
      </div>
          ...
          <div style={{ width: 440, background: "#181B24", border: "1px solid rgba(255,255,255,.1)",
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 600,
                color: "rgba(255,255,255,.5)", marginBottom: 7, letterSpacing: "0.03em" }}>
                账号（邮箱或用户名）
              </label>
              <div style={{ position: "relative" }}>
                ...
                <input
                  ...
                  style={{ width: "100%", padding: "11px 12px 11px 34px", borderRadius: 12, fontSize: 13.5,
                    ...
                    color: "white", outline: "none", boxSizing: "border-box", transition: "border-color .15s" }}
            ...
            <div style={{ marginBottom: 24 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 600,
                color: "rgba(255,255,255,.5)", marginBottom: 10, letterSpacing: "0.03em" }}>
                分配职能
              </label>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {TEAM_ROLES.map(r => {
                  ...
                  return (
                    <button key={r} onClick={() => setRole(r)}
                      ...
                      <div style={{ textAlign: "left" }}>
                        <div style={{ fontSize: 13, fontWeight: 600, color: selected ? rs.color : "rgba(255,255,255,.6)" }}>{r}</div>
                      </div>
                    </button>
                  );
                })}
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={handleInvite}
                ...
                style={{ flex: 2, padding: "11px", borderRadius: 12, fontSize: 13.5, fontWeight: 700,
                  ...
                  color: sent ? "#34d399" : "black",
                  opacity: !account.trim() ? 0.4 : 1,
                  display: "flex", alignItems: "center", justifyContent: "center", gap: 8, cursor: "none",
                  transition: "all .3s" }}>
...
function PlotAnalysisDetailPage({ navigate }: Nav) {
  // Legacy route — redirect to the unified 9-step workflow
  useEffect(() => { navigate("plot-analysis"); }, []);
...
export default function App() {
  ...
  return (
    <AppLayout currentPage={activePage} navigate={navigate}>
      ...
      {(activePage === "plot-analysis" || activePage === "plot-analysis-detail") && <PlotAnalysisPage navigate={navigate} />}
